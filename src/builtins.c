...
void jl_static_show_typeof(JL_STREAM *out, jl_value_t *v)
{
    if (v == NULL) {
        ...
        jl_printf(out, "%s", jl_symbol_name_(dv->name->name));
        if (dv->parameters && (jl_value_t*)dv != dv->name->primary) {
            size_t j, tlen = jl_nparams(dv);
            if (tlen > 0) {
                jl_printf(out, "{");
...
JL_DLLEXPORT void jlt_(void *jl_value)
{
    jl_in_jl_++;
    JL_TRY {
...
int jl_arg_is_typetype_oftype(jl_value_t *args, int indx, char* jltypename)
{
    ...
    if (!jl_is_datatype(T))
        return 0;
    return ((jl_datatype_t*)T)->name->name == jl_symbol(jltypename);
}
