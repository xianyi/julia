 ...
 *         kill-use = get-unique-use<isa-CallInst>(c)
 *         bb-uses[bb][c] = kill
 */
    std::map<BasicBlock*, std::map<frame_register, liveness::id> > bb_uses;
...
//                     // TODO: track variable liveness information
//                }
//                else {
//#ifdef LLVM35
//                    assert(use->getOperandNo() == StoreInst::getPointerOperandIndex());
//#else
//                    assert(use.getOperandNo() == StoreInst::getPointerOperandIndex());
//#endif
//                }
    ...
    for (Function::iterator bb = F.begin(), be = F.end(); bb != be; ++bb) {
        for (BasicBlock::iterator i = bb->begin(), ie = bb->end(); i != ie; ) {
            ...
            if (StoreInst *storeInst = dyn_cast<StoreInst>(inst)) {
                frame_register def = get_gcroot(storeInst->getPointerOperand());
                CallInst *gcroot = def.first;
                if (gcroot && gcroot->getCalledFunction() == jlcall_frame_func) {
